...
public class RESTclient {
	...
	public double surfaceSide(String address, int port, float value1, float value2) {
		...
		Response respuesta = (Response) client.target( "http://"+address+":"+port+"/cil/surfaceSide/"+value1+"/"+value2).request("application/json").get();

		com.google.gson.Gson gson = new com.google.gson.Gson();

		Double respuestaDouble = gson.fromJson(respuesta.readEntity(String.class),Double.class);
	...
	public Surface surfaceTotal(String address, int port, float value1, float value2) {
		...
		Response respuesta = (Response) client.target( "http://"+address+":"+port+"/cil/surfaceTotal/"+value1+"/"+value2).request("application/json").get();

		com.google.gson.Gson gson = new com.google.gson.Gson();

		Surface respuestaSurface = gson.fromJson(respuesta.readEntity(String.class),Surface.class);
	
		return respuestaSurface;
	}
}